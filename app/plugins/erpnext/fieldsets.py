...
- auto_fill_hints: fields ERPNext auto-populates on save or that have safe defaults

Policy rules (machine-readable; LLMs must follow these):
1. Fieldsets are advisory. Live doctype metadata (erpnext_get_doctype_meta) is
   authoritative for write validation.
2. Fields listed in ``auto_fill_hints`` MUST be omitted from creation payloads.
   Do not include them in the request body; ERPNext will derive or default them
   automatically.
3. Attempt-first: attempt document creation with the minimum required payload
   first. Only ask the user for a field if ERPNext returns a MandatoryError that
   explicitly names that specific field. Never pre-empt by asking the user for
   auto-fill fields before attempting the create.
4. When ERPNext does reject with a MandatoryError for a field listed in
   ``auto_fill_hints``, use the ``safe_default`` value from the hint as the
   fallback (if provided) and retry before escalating to the user.
"""
...
FIELDSETS: dict[str, dict] = {
    "Sales Order": {
        ...
        "auto_fill_hints": [
            {"field": "company", "reason": "Auto-set from system defaults when only one company exists. Omit from payload; only provide if ERPNext returns a MandatoryError naming this field."},
            {"field": "naming_series", "reason": "Auto-generated by ERPNext naming rules. Never include in payload."},
            {"field": "transaction_date", "reason": "Defaults to today."},
        ],
        "required": [
    ...
    "Sales Invoice": {
        ...
        "auto_fill_hints": [
            {"field": "company", "reason": "Auto-set from system defaults when only one company exists. Omit from payload; only provide if ERPNext returns a MandatoryError naming this field."},
            {"field": "debit_to", "reason": "Auto-set by ERPNext from the customer's receivable account. Omit from payload."},
            {"field": "naming_series", "reason": "Auto-generated by ERPNext naming rules. Never include in payload."},
        ],
        "required": [
    ...
    "Item": {
        ...
        "auto_fill_hints": [
            {"field": "item_group", "reason": "Has a system-wide default. Omit from payload; if ERPNext rejects with MandatoryError, use safe_default.", "safe_default": "All Item Groups"},
            {"field": "stock_uom", "reason": "Has a system-wide default. Omit from payload; if ERPNext rejects with MandatoryError, use safe_default.", "safe_default": "Nos"},
        ],
        "required": [
    ...
    "Supplier": {
        ...
        "auto_fill_hints": [
            {"field": "supplier_type", "reason": "Has a system-wide default. Omit from payload; if ERPNext rejects with MandatoryError, use safe_default.", "safe_default": "Company"},
        ],
        "required": [
    ...
    "Purchase Invoice": {
        ...
        "auto_fill_hints": [
            {"field": "company", "reason": "Auto-set from system defaults when only one company exists. Omit from payload; only provide if ERPNext returns a MandatoryError naming this field."},
            {"field": "credit_to", "reason": "Auto-set by ERPNext from the supplier's payable account. Omit from payload."},
            {"field": "naming_series", "reason": "Auto-generated by ERPNext naming rules. Never include in payload."},
        ],
        "required": [
